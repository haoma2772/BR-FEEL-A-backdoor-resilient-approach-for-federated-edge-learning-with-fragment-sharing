...
poison_seed = config['general']['random_seed']
...
class poison_generator():
    ...
    def __init__(self, img_size, dataset, poison_rate, path, trigger_names, alphas, target_class=0, cover_rate=0.01):
        ...
        for i in range(len(trigger_names)):
            trigger_path = os.path.join(base_config.triggers_dir, trigger_names[i])
            trigger_mask_path = os.path.join(base_config.triggers_dir, 'mask_%s' % trigger_names[i])
            ...
            if os.path.exists(trigger_mask_path):  # if there explicitly exists a trigger mask (with the same name)
                ...
                trigger_mask = trigger_transform(trigger_mask)  # only use 1 channel
    ...
    def generate_poisoned_training_set(self):
        ...
        # 还有点小问题
        num_cover = min(int(self.num_img * self.cover_rate), self.num_img - num_poison)
...
class poison_transform():
    ...
    def __init__(self, img_size, test_trigger_names, test_alphas, target_class=0, denormalizer=None, normalizer=None):
        ...
        for i in range(len(test_trigger_names)):
            trigger_path = os.path.join(base_config.triggers_dir, test_trigger_names[i])
            trigger_mask_path = os.path.join(base_config.triggers_dir, 'mask_%s' % test_trigger_names[i])
            trigger = Image.open(trigger_path).convert("RGB")
            trigger = trigger_transform(trigger)
            if os.path.exists(trigger_mask_path):  # if there explicitly exists a trigger mask (with the same name)
